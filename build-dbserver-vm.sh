...
echo "CREATE DATABASE hvlofi;" | mysql
...
# the "hvlofi" database that we just created, above.
echo "GRANT ALL PRIVILEGES ON hvlofi.* TO 'webuser'@'%'" | mysql

echo "CREATE USER 'posuser'@'%' IDENTIFIED BY 'posaccount';" | mysql

echo "GRANT ALL PRIVILEGES ON hvlofi.* TO 'posuser'@'%'" | mysql
...
# the user to connect as (webuser) and the database to use (hvlofi).
cat /vagrant/setup-database.sql | mysql -u webuser hvlofi
